...
'''
save_path = "/home/compu/ymh/FSMT/save/new_result38/"
model_name = "/home/compu/ymh/FSMT/save/" + "generator38.pth"
...
for iteration in range(total):
    ...
    fake = generator(real_joint, real, noise)
    
    real_validity = discriminator(real, real_joint)
    fake_validity = discriminator(fake.detach(), real_joint)
    
    d_real = 0.5 * torch.mean((real_validity-ones)**2)
    d_fake = 0.5 * torch.mean((fake_validity-zeros)**2)
    ...
    fake_validity = discriminator(fake, real_joint)
    g_loss = 0.5 * torch.mean((fake_validity-ones)**2)
    ...
    if iteration % save == 0:
        save_image(real_joint.data[:4], os.path.join(save_path, "%06d_1input.png" % iteration), nrow=2, normalize=True, range=(-1, 1))
