...
'''
save_path = "/home/compu/ymh/FSMT/save/new_result59/"
model_name = "/home/compu/ymh/FSMT/save/" + "generator59.pth"
...
pre_tripple_loader_1 = DataLoader(tripple1, batch_size=8, shuffle=True)

#reference_from = reference_set(path2, (num_ref-batch_size), True)
pre_reference_from_1 = reference_set(path1_2, 8, True)
...
#tripple_loader = DataLoader(tripple, batch_size=batch_size, shuffle=True)
pre_tripple_loader_2 = DataLoader(tripple2, batch_size=8, shuffle=True)

#reference_from = reference_set(path2, (num_ref-batch_size), True)
pre_reference_from_2 = reference_set(path2_2, 8, True)
...
#tripple_loader = DataLoader(tripple, batch_size=batch_size, shuffle=True)
pre_tripple_loader_3 = DataLoader(tripple3, batch_size=8, shuffle=True)

#reference_from = reference_set(path2, (num_ref-batch_size), True)
pre_reference_from_3 = reference_set(path3_2, 8, True)
...
#tripple_loader = DataLoader(tripple, batch_size=batch_size, shuffle=True)
pre_tripple_loader_4 = DataLoader(tripple4, batch_size=8, shuffle=True)

#reference_from = reference_set(path2, (num_ref-batch_size), True)
pre_reference_from_4 = reference_set(path4_2, 8, True)
...
#tripple_loader = DataLoader(tripple, batch_size=batch_size, shuffle=True)
pre_tripple_loader_5 = DataLoader(tripple5, batch_size=8, shuffle=True)

#reference_from = reference_set(path2, (num_ref-batch_size), True)
pre_reference_from_5 = reference_set(path5_2, 8, True)
...
#tripple_loader = DataLoader(tripple, batch_size=batch_size, shuffle=True)
pre_tripple_loader_6 = DataLoader(tripple6, batch_size=8, shuffle=True)

#reference_from = reference_set(path2, (num_ref-batch_size), True)
pre_reference_from_6 = reference_set(path6_2, 8, True)
...
for pre_iteration in range(100000):
    ...
    if which == 0:
        joint, real, semantic, mask_num = pre_tripple_loader_1.__iter__().next()
        reference = pre_reference_from_1.__getitem__(0)
    elif which == 1:
        joint, real, semantic, mask_num = pre_tripple_loader_2.__iter__().next()
        reference = pre_reference_from_2.__getitem__(0)
    elif which == 2:
        joint, real, semantic, mask_num = pre_tripple_loader_3.__iter__().next()
        reference = pre_reference_from_3.__getitem__(0)
    elif which == 3:
        joint, real, semantic, mask_num = pre_tripple_loader_4.__iter__().next()
        reference = pre_reference_from_4.__getitem__(0)
    elif which == 4:
        joint, real, semantic, mask_num = pre_tripple_loader_5.__iter__().next()
        reference = pre_reference_from_5.__getitem__(0)
    elif which == 5:
        joint, real, semantic, mask_num = pre_tripple_loader_6.__iter__().next()
        reference = pre_reference_from_6.__getitem__(0)
    else:
        print("which wrong")
    ...
    if pre_iteration % regularize == 0:
        ...
        print("[Regularize]", end=" ")
    else:
        fake = generator(joint, style[shuffle])
    
    l1_loss = (100 * torch.abs((fake * semantic - real * semantic))).reshape(8,-1)
    l1_loss = l1_loss.sum(dim=1, keepdim=True)
    l1_loss = l1_loss/mask_num
    l1_loss = l1_loss.mean()
    
    global_l1_loss = L1_loss(fake, real) * 50
    
    loss = l1_loss + global_l1_loss
    loss.backward()
    optimizer_G.step()
