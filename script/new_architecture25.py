...
class Attention_style(nn.Module):
    def __init__(self, in_channel, out_channel, downsample=True):
        ...
        self.conv1 = nn.utils.spectral_norm(nn.Conv3d(in_channel, mid_channel, kernel_size=1, stride=1, padding=0, bias=False))
        self.conv2 = nn.utils.spectral_norm(nn.Conv3d(in_channel, mid_channel, kernel_size=1, stride=1, padding=0, bias=False))
        self.conv3 = nn.utils.spectral_norm(nn.Conv3d(in_channel, mid_channel, kernel_size=1, stride=1, padding=0, bias=False))
        self.conv4 = nn.utils.spectral_norm(nn.Conv3d(mid_channel, out_channel, kernel_size=3, stride=1, padding=1, bias=False))
        self.conv5 = nn.utils.spectral_norm(nn.Conv3d(in_channel, out_channel, kernel_size=1, stride=1, padding=0, bias=False))
...
class Generator(nn.Module):
    ...
    def forward(self, landmark, style, noise):
        ...
        ref = torch.cat((landmark.expand_as(style), style), dim=1)
        ref = ref.permute(1,0,2,3).unsqueeze(0)
        style0 = self.style0(ref)
        style0 = style0.squeeze(0).permute(1,0,2,3)
        style0 = style0.max(dim=0, keepdim=True)[0]
...
if __name__ == "__main__":
    ...
    a1 = torch.randn((1,3,320,192)).to(device)
    a2 = torch.randn((8,3,320,192)).to(device)
