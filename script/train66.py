...
from new_architecture30 import Generator, Discriminator
from new_dataset3 import pair_set
from ContentLoss import LossCnt, LossAdv
...
for iteration in range(total):
    ...
    for i in range(n_critic):
        ...
        real_validity, _ = discriminator(real, joint)
        fake_validity, _ = discriminator(fake.detach(), joint)
        
        alpha = Tensor(np.random.random((batch_size, 1, 1, 1)))
        interpolates = (alpha * real.data + ((1 - alpha) * fake.data)).requires_grad_(True)
    
        d_interpolates, _ = discriminator(interpolates, joint)
    ...
    _, real_feature = discriminator(real, joint)
    fake_validity, fake_feature = discriminator(fake, joint)
    
    adv_loss = Adv_loss(fake_validity, fake_feature, real_feature)
    l1_loss = L1_loss(fake, real) * 5
