...
def _weights_init(m):
    if isinstance(m, nn.Conv2d):
        ...
        m.weight.data = m.weight.data/math.sqrt(shape0*shape1*shape2*shape3*100)
    ...
    if isinstance(m, nn.Linear):
        ...
        shape0 = m.weight.data.shape[0]
        shape1 = m.weight.data.shape[1]
        m.weight.data = m.weight.data/math.sqrt(shape0*shape1*100)
        
        if m.bias is not None:
            torch.nn.init.zeros_(m.bias)

class Normalized_Conv2d(nn.Module):
    def __init__(self, in_ch, out_ch, kernel_size, stride=1, padding=0, groups=1, bias=True):
        ...
        self.weight = nn.Parameter(torch.randn(out_ch, int(in_ch//groups), kernel_size, kernel_size))
        #print(self.weight.shape)
        
        self.scale = 1 / math.sqrt(int(in_ch//groups) * kernel_size * kernel_size)
        ...
        #self.std = nn.Parameter(torch.rand(1, int(in_ch//groups), 1, 1))
        #self.mean = nn.Parameter(torch.randn(1, int(in_ch//groups), 1, 1))
        
    def forward(self, x):
        #C = self.weight.shape[1]
        #mean = self.weight.permute(1,0,2,3).reshape(C,-1).mean(dim=1).reshape(1,C,1,1)
        #std = self.weight.permute(1,0,2,3).reshape(C,-1).std(dim=1).reshape(1,C,1,1)
        
        #weight = (self.weight - mean) / (std + self.eps)
        
        weight = self.weight * self.scale
        #weight = self.std * weight + self.mean
        #print(weight.shape)
...
class EqualLinear(nn.Module):
    ...
    def forward(self, input):
        if self.activation:
            ...
            out = F.linear(
                input, weight = self.weight * self.scale, bias = self.bias
            )
...
class Feature(nn.Module):
    def __init__(self, in_channel, eps = 1e-6):
        ...
        self.fc2 = EqualLinear(512, in_channel, activation=True)
...
class UpBlock(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        self.conv1 = nn.Sequential(
            ...
            Normalized_Conv2d(in_channel, in_channel, kernel_size = 3, bias=False)
            )
        self.conv2 = nn.Sequential(
            nn.ReflectionPad2d(1),
            Normalized_Conv2d(in_channel, out_channel, kernel_size = 3, bias=False)
            )
...
class RGBBlock(nn.Module):
    def __init__(self, in_channel):
        ...
        self.conv = nn.Sequential(
            nn.ReflectionPad2d(1),
            Normalized_Conv2d(in_channel, 3, kernel_size = 3, bias=True)
            )
...
class Attention(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        self.conv1 = Normalized_Conv2d(in_channel, mid_channel, kernel_size=1, stride=1, padding=0, bias=False)
        self.conv2 = Normalized_Conv2d(in_channel, mid_channel, kernel_size=1, stride=1, padding=0, bias=False)
        self.conv3 = Normalized_Conv2d(in_channel, mid_channel, kernel_size=1, stride=1, padding=0, bias=False)
        self.conv4 = Normalized_Conv2d(mid_channel, out_channel, kernel_size=1, stride=1, padding=0, bias=False)
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.init_param = nn.Parameter(torch.randn((1,512,4,4))/math.sqrt(512*4*4))
    ...
    def forward(self, style):
        ...
        out = self.init_param.expand((style.shape[0],512,4,4))
...
class ResBlockDown(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        #right
        self.conv_r1 = Normalized_Conv2d(in_channel, out_channel, 3, 1, 1, bias=False)
        self.conv_r2 = Normalized_Conv2d(out_channel, out_channel, 3, 1, 1, bias=False)
...
class ResBlockD(nn.Module):
    def __init__(self, in_channel):
        ...
        #left
        self.conv1 = Normalized_Conv2d(in_channel, in_channel, 3, padding = 1)
        self.conv2 = Normalized_Conv2d(in_channel, in_channel, 3, padding = 1)
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.fc2 = nn.Sequential(
            EqualLinear(512, 256),
            EqualLinear(256, 1, bias=False, activation=False)
            )
