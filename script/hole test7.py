...
for epoch in range(200):
    for i, (imgs, _) in enumerate(dataloader):
        ...
        g_loss = adversarial_loss(discriminator(gen_imgs), ones)
        ...
        n1_loss = adversarial_loss(discriminator(next_imgs1), ones)
        ...
        n2_loss = adversarial_loss(discriminator(next_imgs2), ones)
        ...
        n3_loss = adversarial_loss(discriminator(next_imgs3), ones)
        ...
        next1_loss = adversarial_loss(discriminator(next_imgs1.detach()), zeros)
        next2_loss = adversarial_loss(discriminator(next_imgs2.detach()), zeros)
        next3_loss = adversarial_loss(discriminator(next_imgs3.detach()), zeros)
        ...
        if batches_done % 400 == 0:
            save_image(gen_imgs.data[:25], "images7/%d.png" % batches_done, nrow=5, normalize=True)