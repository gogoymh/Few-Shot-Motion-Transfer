...
class ResBlock(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        self.conv1 = nn.utils.spectral_norm(nn.Conv2d(in_channel, out_channel, 3, 1, 1, bias=False))
        self.conv2 = nn.utils.spectral_norm(nn.Conv2d(out_channel, out_channel, 3, stride, 1, bias=False))
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.init = nn.utils.spectral_norm(nn.Conv2d(6, 16, 7, 1, 3, bias=False))
        
        self.block1 = self.make_layer(4, 16, 32)
        self.block2 = self.make_layer(8, 32, 64)
        self.block3 = self.make_layer(16, 64, 128)
        self.block4 = self.make_layer(16, 128, 256)
        self.block5 = self.make_layer(32, 256, 512)
