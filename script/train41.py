...
save_path = "/data1/ymh/FSMT/save/new_result41/"
model_name = "/data1/ymh/FSMT/save/" + "generator41.pth"

path1 = "/data1/ymh/FSMT/dataset/flip_rendered/"
path2 = "/data1/ymh/FSMT/dataset/flip/"

'''
save_path = "/home/compu/ymh/FSMT/save/new_result41/"
model_name = "/home/compu/ymh/FSMT/save/" + "generator41.pth"
...
for iteration in range(total):
    ...
    if iteration % n_critic == 0:
        ...
        if l1_loss.item() >= 2:
            ...
            l1_loss.backward()
            optimizer_G.step()
            print("[L1 loss:%f]" % l1_loss.item(), end=" ")
        elif l1_loss.item() < 2 and l1_loss.item() >= 1.5:
            n_critic = 5
            l1_loss.backward()
            optimizer_G.step()
            print("[L1 loss:%f]" % l1_loss.item(), end=" ")
            ...
            g_loss = -torch.mean(fake_validity, joint) + l1_loss
            g_loss.backward()
            optimizer_G.step()
            print("[g loss:%f]" % g_loss.item(), end=" ")
    
        print("[D loss: %f] [Penelty: %f]" % (d_loss.item(), gradient_penalty.item()))    
